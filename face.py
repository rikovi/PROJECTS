...
def detect_faces(input_img_path):
	...
	prototxt_path = r"C:\Users\komal\Downloads\AI_project\code\deploy_prototxt.txt"
	model_path = r"C:\Users\komal\Downloads\AI_project\code\res10_300x300_ssd_iter_140000.caffemodel"
...
def find_landmarks(input_img_path, face):
	...
	shape_predictor_path = r"C:\Users\komal\Downloads\AI_project\code\shape_predictor_68_face_landmarks.dat"
...
# 	if midpoint is None or angle is None or scale is None:print("Error: Invalid transformation parameters.")
# 	else:M = cv2.getRotationMatrix2D(midpoint, angle, scale)
...
def align_face(input_img_path, landmarks):
	# Define an ordered dictionary for the facial landmarks.
	FACIAL_LANDMARKS_68_IDXS = OrderedDict([
		("mouth", (48, 68)),
		("right_eyebrow", (17, 22)),
		("left_eyebrow", (22, 27)),
	...
	# Desired eye positioning and face size.
	left_eye = 0.35
	right_eye = 1.0 - left_eye
	face_size = 100
	...
	# Calculate center of mass for each eye.
	left_eye_com = left_eye_pts.mean(axis=0).astype("int")
	right_eye_com = right_eye_pts.mean(axis=0).astype("int")
	...
	# Compute midpoint as a tuple of integers.
	midpoint = (int((left_eye_com[0] + right_eye_com[0]) / 2), 
	            int((left_eye_com[1] + right_eye_com[1]) / 2))

	# Check midpoint, angle, and scale before proceeding.
	print("Midpoint:", midpoint)
	print("Angle:", angle)
	print("Scale:", scale)

	if midpoint is None or angle is None or scale is None:
		print("Error: Invalid transformation parameters.")
		return None
